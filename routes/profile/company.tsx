...
import { backendApi, webUrl } from "#util/globals.ts";
...
export const handler: Handlers<any | null> = {
  async GET(req, context) {
    ...
    if (id && Number(id) > 0) {
      const entityResp = await fetch(`${backendApi}/company?id=${id}`);
  ...
  async POST(req, context) {
    ...
    const entityResp = await fetch(`${backendApi}/company`, {
      method,
...
export default function Index(props: PageProps) {
  ...
  navListSidebar2.push({
    href: webUrl("personal/service", url.searchParams),
      ...
      <div class="frame-form">
        ...
        <article class="flex flex-col mt-28">
          <div class="ml-8 font-bold text-lg">
            {isBlank ? "New Company" : "Edit Company"}
          </div>
